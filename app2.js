App = {
    ...
    show_evi: async function(cid,times) {
        var i,j;
        App.contracts.Cert.deployed().then(function(instance){
            return instace.show_case_num().call()
        })
        .then(function(result){
            for(i=1; i<result[0]; i++)
            {
                App.contracts.Cert.deployed().then(function(instance){
                    return instance.show_evi_num(i).call();
                })
                .then(function(result2){
                    for(j=0; j<result2[0]; j++)
                    {
                        App.contracts.Cert.deployed().then(function(instance){
                            return instance.show(i,j).call();
                        })
                        .then(function(result3){
                            document.getElementById('show_block').insertAdjacentHTML("afterend", "<p> case num:"+i+" name:"+result3[0]+" hash:"+result3[1]+" signer:"+result3[2]+"</p>");
                        })
                    }     
                })
            }
